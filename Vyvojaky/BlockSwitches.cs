...
namespace Vyvojaky
{
    internal class BlockSwitches : PictureBox
    {
        ...
        public BlockSwitches(int pocetCasu, string vstupString, List<TextBox> boxy)
        {
        ...
        public void Kontrola(TextBox console)
        {
            ...
            //bool promìnná pro pøípady, že nenajde cílenou promìnnou nebo se stane i jiná chyba
            bool check = false;
            ...
            if (typVstupu != null)
            {
                if (typVstupu == "Int16" || typVstupu == "Int32" || typVstupu == "Int64")
                {
                    ...
                    if (check)
                        MessageBox.Show("Nìco se nepovedlo!");
                    else
                    {
                        for (int i = 0; i < hodnoty.Count; i++)
                        {
